...
public sealed class CourtPhoto
{
    private CourtPhoto() { }

    public CourtPhoto(Guid id, Guid courtId, string imageUrl, int displayOrder,
        bool isPrimary, DateTimeOffset createdAt, string? altText = null)
    {
        ...
        if (courtId == Guid.Empty) throw new ArgumentException("A venue is required.", nameof(courtId));
